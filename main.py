    ...
    if len(conts) != 0:
        ...
        if len(conts) != 0:
            for obj in conts2:
                ...
                nW = round((utilis.findDis(nPoints[0][0]//scale,nPoints[1][0]//scale)/10),1)
                nH = round((utilis.findDis(nPoints[0][0]//scale,nPoints[2][0]//scale)/10),1)
                ...
                cv2.putText(imgContours2, '{}cm'.format(nH), (x - 70, y + h //2), cv2.FONT_HERSHEY_COMPLEX,1,
                            (0, 0, 0), 2)
        imgContours2= cv2.resize(imgContours2, (0, 0), None, 0.7, 0.7)
